...
"timeGap")
	...
	hadoop jar $CHOMBO_JAR_NAME  $CLASS_NAME -Dconf.path=$PROP_FILE -Dconf.group=tgs $IN_PATH  $OUT_PATH
...
"timeGapStats")
	...
	hadoop fs -rmr $OUT_PATH
	echo "removed output dir $OUT_PATH"
	hadoop jar $CHOMBO_JAR_NAME  $CLASS_NAME -Dconf.path=$PROP_FILE  -Dconf.group=nas $IN_PATH  $OUT_PATH
...
"lifeCycle")
	...
	hadoop fs -rmr $OUT_PATH
	echo "removed output dir $OUT_PATH"
	hadoop jar $JAR_NAME  $CLASS_NAME -Dconf.path=$PROP_FILE -Dconf.group=tgs,lcf,1 $IN_PATH  $OUT_PATH
...
"lifeCycleStats")
	...
	hadoop fs -rmr $OUT_PATH
	echo "removed output dir $OUT_PATH"
	hadoop jar $CHOMBO_JAR_NAME  $CLASS_NAME -Dconf.path=$PROP_FILE -Dconf.group=nas,1 $IN_PATH  $OUT_PATH
